#!/usr/bin/env elixir
# Usage: elixir scripts/run_solution.exs <slug-or-id> [--lang elixir|go|python|js]
...
defmodule LeetcodeTracker.RunSolution do
  ...
  def run(argv) do
    {opts, args, _} = OptionParser.parse(argv, strict: [lang: :string])
    arg = List.first(args) || raise "usage: elixir scripts/run_solution.exs <slug-or-id> [--lang elixir|go|python|js]"
    lang = opts[:lang] || "elixir"

    {lc_slug, ext} =
      Map.get(@lang_map, lang) ||
        raise "unknown lang '#{lang}'. choose from: #{Map.keys(@lang_map) |> Enum.join(", ")}"
