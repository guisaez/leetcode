...
var rob = function(nums){
    ...
    var robPattern1 = robHelper(nums, 0, nums.length - 2);
    var robPattern2 = robHelper(nums, 1, nums.length - 1);

    return Math.max(robPattern1, robPattern2);
    
}

var robHelper = function(nums, startHouse, endHouse){
    ...
    money[startHouse] = nums[startHouse];
    money[startHouse + 1] = Math.max(nums[startHouse], nums[startHouse + 1]);

    for(let i = startHouse + 2; i <= endHouse; i++){
        money[i] = Math.max(money[i - 2] + nums[i], money[i - 1]);
    }

    return money[endHouse]
}